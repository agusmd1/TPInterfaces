...
let filaCampo = document.querySelector(".fila-campo");
...
//limpiar la primer fila:
let limpiar = document.querySelector("#limpiar");
limpiar.addEventListener("click", () => {
    let fila = limpiar.parentElement;
...
function insertarPersonas() {
    ...
    for (let index = 0; index < nombres.length; index++) {
        let nombre = nombres[index].value;
        let apellido = apellidos[index].value;

        nombreApellido = document.createElement("p");
        nombreApellido.innerHTML = apellido + ", " + nombre;

        personas.appendChild(nombreApellido);
    }
};
