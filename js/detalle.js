...
const $popup = document.getElementById('popup_mostrar');
const $otros_cursos = document.getElementById('otros_cursos');
let data = [];
...
async function getData() {
    try {
        ...
        const curso = data.find(curso => curso.id == CursoID);
        console.log(CursoID);
        Cargar_detalle(curso, $detalles);
        const recomendados = recomendadosAleatorio(data, curso.id-1, 4);
        Cargar_otros_cursos(recomendados,$otros_cursos);
        Cargar_popup(curso,$popup);
    } catch (error) {
        console.log(error);
    }
}
...
function recomendadosAleatorio(cursos, menos, num) {
  const filterArray = cursos.filter(curso => curso.id !== menos);
  const result = [];
  const arrayLength = filterArray.length;
  while (result.length < num) {
    const numAleatorio = Math.floor(Math.random() * arrayLength);
    const elementoAleatorio = filterArray[numAleatorio];
    if (!result.includes(elementoAleatorio)) {
      result.push(elementoAleatorio);
    }
  }
  return result;
}
///Otros cursos
function Cargar_otros_cursos(array, contenedor) {
  contenedor.innerHTML = '';
  array.forEach(curso => {
      let div = document.createElement('div');
      div.classList.add('producto');
      div.innerHTML = `<div class ="curso${curso.id}">
      <div class="imagen">
      <img src="${curso.imagen}" alt="imagen de ${curso.titulo}">
      <p class="precio">${curso.precio} ARS</p>
      <p class="horas">${curso.tiempo} hs</p>
      </div>
      <div class="producto_info">
      <div class="producto_detalle">
      <h4>${curso.titulo}</h4>
      <a href="/detalle.html?id=${curso.id}">Ver detalle</a>
      </div>
      </div>
      </div>
      `;
...
//cargar tarjeta
function Cargar_detalle(curso, container) { 
    container.innerHTML = '';
    let div = document.createElement('div');
    div.classList.add('info_curso');
    div.innerHTML = `<div class="imagen_curso">
        <img src="${curso.imagen}" alt="${curso.titulo}">
    </div>
    <div class="info_general">
        <h1>${curso.titulo}</h1>
        <p>Precio: ARG ${curso.precio}</p>
        <p>Duración: ${curso.tiempo} hs</p>
        <p>Descripción: ${curso.descripcion}</p>
        <p>Requisitos previos: ${curso.requisitos_previos}</p>
        <button class="boton" id="inscribirse">INSCRIBIRSE</button>
    </div>`;
    container.appendChild(div);
    
    const botonInscribirse = document.getElementById('inscribirse');
    if (botonInscribirse) {
        botonInscribirse.addEventListener('click', click_Inscribirse);
    }
}
/////////


//contenidos por clase (acordeon):
let unidades = document.querySelectorAll(".unidad");

unidades.forEach((i) => {
    i.addEventListener("click", () => {
        i.nextElementSibling.classList.toggle("d-none");
        i.classList.toggle("contraer-contenido");
    });
});
////////

//para el popup:
function Cargar_popup(curso, container) {
    container.innerHTML = '';
      let div = document.createElement('div');
      div.innerHTML =`<div class="popup-info">
      <img src="${curso.imagen}" alt="${curso.titulo}">
      <div class="popup-texto">
          <h4>${curso.titulo}</h4>
          <p>Precio: ${curso.precio}</p>
          <p>Duración: ${curso.tiempo} hs.</p>
          <button class="boton" id="aceptar">ACEPTAR</button>
      </div>
  </div>`;
  container.appendChild(div)
  const botonAceptar = document.getElementById('aceptar');
  if(botonAceptar){
    botonAceptar.addEventListener('click' , click_aceptar);
  }
  }
  function click_Inscribirse() {
