...
//mostrar cantidad de caracteres de la consulta
let comentarios = document.querySelector("#comentarios");
let cantidadCaracteres = document.querySelector(".cantidad-caracteres");
comentarios.addEventListener("keyup",()=>{
    cantidadCaracteres.style.color = "#000000";
    cantidadCaracteres.innerHTML = comentarios.value.length + "/1000";
    if(comentarios.value.length>1000){
        cantidadCaracteres.style.color = "#ff0000";
    }
}); 

//validar nombre, mail, telefono y largo de la consulta
function validar(){
    let error = false;
    let mensajeError = "";
    ...
    if(!regexEmail.test(email)){
        error = true;
        mensajeError += "<p>El email debe tener un formato válido.</p>";
    }
    
    if(telefono.includes("-")){
        if(!regexTelefono2.test(telefono)){
            error = true;
            mensajeError += "<p>El teléfono debe tener un formato válido. Ej: 1234-5678</p>";
        }
    }else if(telefono != "" && !regexTelefono1.test(telefono)){
        error = true;
        mensajeError += "<p>El teléfono no puede contener letras.</p>";
    }

    if(comentarios.length == 0){
        error = true;
        mensajeError += "<p>Ingresá una consulta.</p>"
    }
    if(comentarios.length>1000){
        error = true;
        mensajeError += "<p>Excediste el límite de caracteres.</p>"
    }
